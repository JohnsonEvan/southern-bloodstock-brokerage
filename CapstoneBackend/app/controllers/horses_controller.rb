class HorsesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @horse.save
        format.html { redirect_to horse_url(@horse), notice: "Horse was successfully created." }
  ...
  def update
    respond_to do |format|
      if @horse.update(horse_params)
        format.html { redirect_to horse_url(@horse), notice: "Horse was successfully updated." }
  ...
  private
    ...
    def horse_params
      params.require(:horse).permit(:name, :location, :price, :image, :broker)
    end
end
